...
### Отчёты: Протокол + Пересказ (✨)

Действие `summarize` (карусель / меню ⋯ / пайплайн-бар) больше **не** пишет резюме преамбулой в транскрипт. Оно генерирует **два отдельных документа** через встроенные промты и кладёт их в папку сессии:

- `<base>_protocol.md` — Протокол (формальные пункты: повестка, решения, задачи-таблица, открытые вопросы), промт `PROMPT_PROTOCOL`
- `<base>_recap.md` — Пересказ (связный нарратив «о чём говорили»), промт `PROMPT_RECAP`

`<base>` = имя папки сессии (`reportBaseName(transcriptFile)` снимает `_dg/_ai/_groq` и расширение; `reportFileName(file, kind)` строит имя). Оба промта **на русском** (см. правило «AI-промты остаются на русском») и используют `{{transcript}}`.

- `generateReport(kind, transcriptFile, dirHandle, model)` — читает транскрипт (срезает возможную старую преамбулу), подмешивает описания скриншотов через `mergeScreenshotLines`, вызывает модель, пишет чистый markdown в `<base>_<kind>.md`.
- Пикер: Протокол / Пересказ / Оба → выбор модели (`pickModel`). `autoProcess` генерит оба.
- Кастомные промты из `prompts/` теперь применяются только в **чате** (`chat-prompt-btn`), не в summarize.
- HTML-экспорт (`toHtml`) собирает документ с вкладками **Протокол · Пересказ · Транскрипция** (только те, где есть контент; транскрипт — документ-стилем без пузырей; всегда светлая тема; печать раскрывает все вкладки). Legacy: старая inline-преамбула в `_dg.txt` показывается как вкладка Протокол.
...
- Mistral → `mistral-medium-latest` модель (pixtral-large-latest удалён 2026-02-27)
